object LowerTypeBounds extends App :
  ...
  // type B refer to a supertype of type A. In most cases, A will be the
  ...
  // This program implements a singly-linked list. Nil represents an empty
  // list with no elements. class NonEmptyList is a node which contains an
  // element of type A (head) and a reference to the rest of the list (tail).
  // The trait List and its subtypes are covariant because we have +A.
  //
  //However, this program does not compile because the parameter elem in
  // prepend is of type A, which we declared covariant. This doesn’t work
  // because functions are contravariant in their parameter types and covariant
  // in their result types.
  //
  //To fix this, we need to flip the variance of the type of the parameter
  // elem in prepend. We do this by introducing a new type parameter B that
  // has A as a lower type bound.
  ...
  // The type bound on the type parameter for prepend allows adding different
  // varieties of swallows and getting a wider type: instead of List[AfricanSwallow],
  // we get a List[Bird].
  //
